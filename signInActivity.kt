...
class signInActivity : AppCompatActivity() {
    ...
    /* The accountclick function happens when the user presses to sign in.
    * Either they press create or sign in, and given their choice
    * the answer makes a new account or immediately accesses the tree chat */
        ...
        auth.createUserWithEmailAndPassword(username, password)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    channelIntent.putExtra("username", "$username")
    ...
    /* signInAccount is for both create and signing in. Depending on whether or not they enter the
    * correct specifications, they can create a new account and sign in */
        ...
        auth.signInWithEmailAndPassword(email, password)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    val channelIntent = Intent(this, channelActivity :: class.java)
                    channelIntent.putExtra("username", "$userNameInfo")
                    startActivity(channelIntent)
    ...
    fun addNewUser(username: String) {
        ...
        val table = fb.child("treechat/users")
        val userTable = table.push()

        //need to add values for input on the create account portion their name and username
        userTable.child("username").setValue(user_name_field.text.toString())
        userTable.child("name").setValue(actual_name.text.toString())
